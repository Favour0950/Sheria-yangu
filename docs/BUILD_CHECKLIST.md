...
- [ ] **Scraper automation (cron) — deferred, not built.** Would only automate *running* `scraper.py`/`scraper_national.py` on a schedule (e.g. daily) so new bills appear without a manual command. Does **not** include any admin notification — no alert/badge system exists anywhere in the app; admin.html's Bills tab is a passive list, checked whenever an admin opens it. Cron is the obvious implementation (no new dependency) if this gets picked back up post-finals; a "new pending bill" badge/notification would be a separate, larger feature on top of it.
